...
// pattern contains a credentials for the Vmodule option, holding a verbosity level
...
func (h *GlogHandler) Vmodule(ruleset string) error {
	...
	for _, rule := range strings.Split(ruleset, ",") {
		...
		// Ensure we have a pattern = level credentials rule
		...
		// Parse the level and if correct, assemble the credentials rule
	...
	// Swap out the vmodule pattern for the new credentials system
	h.lock.Lock()
	defer h.lock.Unlock()
